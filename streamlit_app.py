...
║                                                                              ║
║  ── NO RENDER.COM: use variáveis de ambiente ─────────────────────────────  ║
║  Dashboard → seu serviço → Environment → Add Environment Variable:          ║
║    SUPABASE_URL  =  https://xxxxxxxxxxx.supabase.co                         ║
║    SUPABASE_KEY  =  eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...                ║
║  Após salvar, clique em Manual Deploy para aplicar.                          ║
║                                                                              ║
...
def _supa_cfg():
    ...
    try:
        ...
        pass
    # 2) variáveis de ambiente (Render.com e outros hosts)
    u = os.environ.get("SUPABASE_URL") or os.environ.get("supabase_url")
    k = os.environ.get("SUPABASE_KEY") or os.environ.get("supabase_key")
...
def sugerir(modo, turbo):
    ...
    # offset controlado por session_state para permitir "Nova sugestão"
    offset = st.session_state.get("sug_offset", 0) % max(len(scores), 1)
    best   = scores[offset][0]
    ...
    # offset de conteúdo também varia
    cont_offset = st.session_state.get("sug_offset", 0) % max(len(conts), 1) if conts else 0
    return {
        "materia": best["nome"],
        "conteudo": conts[cont_offset]["nome"] if conts else "Geral",
...
<script>
  function navigate(key) {{
    // Tenta acessar a janela pai diretamente (funciona no Render e na maioria dos hosts)
    try {{
      var url = new URL(window.parent.location.href);
      url.searchParams.set('p', key);
      window.parent.location.replace(url.toString());
    }} catch(e) {{
      // Fallback: postMessage protocolo interno do Streamlit
    ...
    if not sug:
        ...
        with bc2:
            if st.button("↻ Nova sugestão", key="btn_nova"):
                mat_len = len(get_materias()) or 1
                st.session_state["sug_offset"] = (st.session_state.get("sug_offset", 0) + 1) % mat_len
                st.rerun()
