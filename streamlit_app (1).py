...
# ── BOTTOM NAV (mobile) — injetada via components.html ───────
# st.markdown bloqueia <a href> e tags <style> com seletores externos;
# st.components.v1.html roda num iframe isolado e aceita HTML/JS completo.
...
<style>
  * {{ margin:0; padding:0; box-sizing:border-box; }}
  ...
  .cf-nav-item.cf-nav-active .cf-nav-icon {{
    filter: none;
    transform: scale(1.12);
  }}
  .cf-nav-item.cf-nav-active .cf-nav-lbl {{
    color: #7b96ff;
  }}
  .cf-nav-item.cf-nav-active {{
    background: rgba(91,124,253,.12);
  }}
</style>
</head>
<body>
...
<script>
  function navigate(key) {{
    // Navega na janela pai (o app Streamlit) mudando o query param
...
# CSS extra: esconde sidebar e ajusta padding no mobile (injetado no app principal)
st.markdown("""
<style>
@media (max-width: 767px) {
  section[data-testid="stSidebar"],
  ...
  /* posiciona o iframe da bottom nav fixo na base */
...
@media (min-width: 768px) {
  /* esconde o iframe da bottom nav no desktop */
  iframe[title="streamlit_components_v1_html"] {
    display: none !important;
    height: 0 !important;
  }
